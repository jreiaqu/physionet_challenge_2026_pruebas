...
======================
Define las 5 arquitecturas de deep learning del TFG (SimpleCNN1D, CNNLSTM1D, LSTM,
ResNet1D, TimeSeriesTransformer) y las funciones de entrenamiento, inferencia y checkpointing.

Input:  señales fisiológicas CAISR en ventanas de 30s (9 canales × n_épocas)
Output: probabilidad de deterioro cognitivo por paciente (BCEWithLogitsLoss)
Uso:    importar train_model(), load_model(), run_model(), save_model()
"""
...
# Train your model.
def train_model(data_folder, model_folder, model_type=2, verbose=False, csv_path=DEFAULT_CSV_PATH, n_blocks=2, initial_filters=16, kernel_size=32, pool_size=3, stride=1, padding=2, adaptive_pool=250, dense_units=256, dropout_rate=0.3, max_pool=0, lr=0.0005, bs=16, ne=500, **kwargs): # ** empaqueta diccionario!!! por estar en def
